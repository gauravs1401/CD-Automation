...
def cd_remove(case_number, order):
    ...
    if text_Aruba == 'Aruba':
        ...
        session.findById("wnd[0]/tbar[0]/btn[3]").press()  # Going back to variant "VA02"
        # Dictionary for Fallout Order
        dict_Fallout = {'CASE NUMBER': [case_number], 'HPON': [order], 'COMMENTS': 'FALLOUT/3PAR'}
        df_Fallout = pd.DataFrame(dict_Fallout)
...
conditions = {
    ...
    'FALLOUT/3PAR': PatternFill(start_color='FCE5CD', end_color='FCE5CD', fill_type='solid'),   # Light Pink
